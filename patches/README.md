...
| `mlx_vlm-glm5_next-language.patch` | A `_pool` staleness guard for the DSA indexer after EXO rewinds the KV cache; **absorbed-MLA prefill** (the latent-space form decode already used, for every sequence length -- the expanded per-head K/V cost 64 KiB per context token per 1024-token chunk and OOMed near 100k tokens); a **true sparse prefill** that attends each query only to its indexer-selected keys (O(T) instead of O(T^2); 3.7x faster at 200k tokens); indexer query sub-chunk 512 -> 128. Flags `GLM5_ABSORBED_PREFILL`, `GLM5_SPARSE_PREFILL`, `GLM5_SPARSE_PREFILL_CHUNK`, `GLM5_INDEXER_QUERY_CHUNK`. |
| `mlx_vlm-deepseek_v4-hyper_connection.patch` | Casts the fused hyper-connection kernel's `mixes`/`scale`/`base` to float32 before the call. The kernel type-puns them as float32; every GLM-5.3 / DeepSeek-V4 checkpoint stores them bfloat16, and the sinkhorn renormalises the misread values into a plausible matrix, so the model ran at full speed and silently emitted garbage. Upstream as mlx-vlm#2139. |
...
## How this fork applies them

You do not patch the venv by hand. This fork pins `mlx-lm` and `mlx-vlm` in `pyproject.toml`
`[tool.uv.sources]` to forks that carry these changes as commits (`zhast/mlx-lm` and
`zhast/mlx-vlm`, branch `glm53-release`), so `uv sync` -- and therefore the packaged app,
whose Python is sealed in a PyInstaller archive -- includes them. The files in this
directory are the reference diffs against the upstream baselines (mlx-lm
`rltakashige/mlx-lm` `leo/deepseek-v4` @ 6a3df6cd, mlx-vlm v0.6.17).
